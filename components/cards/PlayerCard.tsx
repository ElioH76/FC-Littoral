 ...
 * Si `hoverPhoto` est fourni, la carte bascule en fondu vers la 2ᵉ pose
 * avec un léger zoom au survol (effet "zoom cinéma").
 */
export function PlayerCard({
  ...
  const hasHover = Boolean(player.photo && player.hoverPhoto);
  ...
  const imgSizes = "(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 20vw";

  return (
    <div className={cn("group relative", hasHover && styles.card)}>
      {/* Cadre visible de la carte (contenu rogné) */}
      <div
        className={cn(
      ...
      >
        <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-b from-forest-600 to-ink">
          ...
          {/* 2ᵉ pose, révélée en fondu au survol */}
          {hasHover && (
            <Image
              src={player.hoverPhoto as string}
              alt=""
              aria-hidden
              fill
              sizes={imgSizes}
              className={cn("object-cover object-top", styles.alt)}
            />
          )}
          ...
          {hasHover && (
            <div
              className={cn(
                "absolute inset-0 bg-gradient-to-t from-ink/60 to-transparent to-55%",
                styles.veil,
              )}
              aria-hidden
            />
          ...
          {/* Numéro */}
          {player.number != null && (
            <span className="absolute left-2 top-2 z-10 inline-flex h-7 min-w-[1.75rem] items-center justify-center rounded-md bg-ink/80 px-1.5 font-display text-sm text-gold backdrop-blur">
              {player.number}
            </span>
          )}
          ...
          {/* Buts */}
          {goals > 0 && (
            <span className="absolute right-2 top-2 z-10 rounded-full bg-gold px-2 py-0.5 font-display text-xs text-ink shadow">
