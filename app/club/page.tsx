...
export default function ClubPage() {
  return (
    <>
      ...
      {/* Valeurs (sombre) */}
      <section className="section bg-ink-800 text-bone">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {club.values.map((value, i) => (
              <Reveal key={value.title} delay={i * 120}>
                <div className="h-full rounded-2xl border border-white/10 bg-ink p-6 transition-all hover:-translate-y-1 hover:border-gold">
                  <div className="font-display text-4xl text-gold">
                    {String(i + 1).padStart(2, "0")}
                  </div>
                  <h3 className="mt-3 font-heading text-lg font-extrabold uppercase tracking-wide text-bone">
                    {value.title}
                  </h3>
                  <p className="mt-2 text-sm text-bone-dim">{value.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
